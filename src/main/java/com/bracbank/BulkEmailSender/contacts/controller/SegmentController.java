...
import com.bracbank.BulkEmailSender.contacts.service.SegmentService;
...
@RestController
@RequestMapping("/contacts/segment")
public class SegmentController extends AppProperty {
    ...
    public AppResponse<Object> filter(
        ...
        try {
            return AppResponse.build(HttpStatus.OK).body(segmentService.filter(currentPage, itemPerPage, params));
        } catch (Exception ex) {
    ...
    public AppResponse<Object> get(@PathVariable("id") long id) {
        Map<String, Object> model = segmentService.findById(id);
        if (model != null) {
    ...
    public AppResponse<Object> update(@PathVariable("id") long id, @RequestBody Map<String, Object> model) {
        ...
        try {
            if (segmentService.update(model)) {
